...
class XLSXReportController(http.Controller):
    @http.route('/xlsx_reports', type='http', auth='user',
                csrf=False)
    def get_report_xlsx(self, model, options, output_format, report_name, applied_on,
                        token='ads'):
        ...
        token = 'dummy_because-api-expects-one'
        try:
            if output_format == 'xlsx':
                ...
                report_object.get_xlsx_report(options, response, applied_on)
